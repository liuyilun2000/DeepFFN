...
from DeepFFNViT.modeling_vit import (
    DeepFFNViTModel,
    DeepFFNViTForMaskedImageModeling,
)

AutoConfig.register("deepffn-vit", DeepFFNViTConfig)
AutoModel.register(DeepFFNViTConfig, DeepFFNViTModel)
AutoModelForMaskedImageModeling.register(DeepFFNViTConfig, DeepFFNViTForMaskedImageModeling)
...
def initialize_model(
    ...
    config = DeepFFNViTConfig(
        ...
        num_labels=21843,  # ImageNet-21k classes
    )
    
    # Initialize model
    model = DeepFFNViTForMaskedImageModeling(config)
